...
sysc = sysc.replace('''    pub fn sys_wait4(args: [u64; 6]) -> SyscallOutcome {
        ...
        loop {
            ...
            crate::sched::with_scheduler(|_s| {
                match crate::sched::sys_reap_child(parent_pid) {''', '''    pub fn sys_wait4(args: [u64; 6]) -> SyscallOutcome {
        let parent_pid = crate::sched::current_process_id();
...
# 4. sys_openat
sysc = sysc.replace('''fn open_path_at(dirfd: i64, path_ptr: usize, flags: u64) -> SyscallOutcome {
...
sysc = sysc.replace('''fn copyin_bytes(ptr: usize, len: usize) -> Result<Vec<u8>, i64> {
    ...
    uaccess::copyin(ptr, &mut bytes).map_err(|e| e.as_errno())?;
    Ok(bytes)
}''', '''fn copyin_bytes(ptr: usize, buf: &mut [u8]) -> Result<(), i64> {
    uaccess::copyin(ptr, buf).map_err(|e| e.as_errno())?;
    Ok(())
}''')
...
sysc = sysc.replace('''fn copyin_c_string(ptr: usize, max_len: usize) -> Result<String, i64> {
    ...
    for index in 0..max_len {
        ...
        let mut byte = [0u8; 1];
        uaccess::copyin(address, &mut byte).map_err(|e| e.as_errno())?;
        if byte[0] == 0 {
