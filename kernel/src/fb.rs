...
static FRAMEBUFFER_PHYSICAL_ADDRESS: AtomicU64 = AtomicU64::new(0);

pub fn set_physical_address(phys_addr: u64) {
    FRAMEBUFFER_PHYSICAL_ADDRESS.store(phys_addr, Ordering::Relaxed);
}

pub fn physical_address() -> Option<u64> {
    let phys = FRAMEBUFFER_PHYSICAL_ADDRESS.load(Ordering::Relaxed);
    if phys != 0 {
        Some(phys)
    } else {
        None
...
pub fn summary() -> Option<FramebufferSummary> {
    unsafe {
        (&*FRAMEBUFFER_CONSOLE.get()).as_ref().map(|console| FramebufferSummary {
            width: console.framebuffer.width,
            height: console.framebuffer.height,
            pitch: console.framebuffer.pitch,
            bpp: console.framebuffer.bpp,
            sample_background: console.background,
            sample_accent: console.accent,
        })
    }
}
...
impl FramebufferConsole {
    ...
    fn draw_glyph(&mut self, x: u64, y: u64, ch: char, color: u32, scale: u64) {
        let glyph = crate::font::lookup_glyph(ch);
